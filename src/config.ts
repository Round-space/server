...
export default cleanEnv(process.env, {
  ...
  PUBLIC_SERVER_URL: str({
    desc: 'Referenece to your public server URL. Replace this when your app is hosted',
    devDefault: 'http://localhost:1337/server',
  }),
  APP_URL: str({
    desc: 'Referenece to your app URL. Replace this when your app is hosted',
  ...
  FROM_ADDRESS: str({
    desc: 'Email address to send emails from',
  }),
  PASSWORD_RESET_TEMPLATE: str({
    desc: 'Sendgrid template id for password reset emails'
  }),
  VERIFICATION_EMAIL_TEMPLATE: str({
    desc: 'Sendgrid template id for verification emails'
  })
});
